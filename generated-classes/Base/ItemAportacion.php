...
/**
 * Base class that represents a row from the 'item_aportacion' table.
 *
 *
 *
* @package    propel.generator..Base
*/
abstract class ItemAportacion implements ActiveRecordInterface
{
    /**
     * TableMap class name
     */
    const TABLE_MAP = '\\Map\\ItemAportacionTableMap';
    ...
    /**
     * Initializes internal state of Base\ItemAportacion object.
     */
    ...
    /**
     * Compares this with another <code>ItemAportacion</code> instance.  If
     * <code>obj</code> is an instance of <code>ItemAportacion</code>, delegates to
     * <code>equals(ItemAportacion)</code>.  Otherwise, returns <code>false</code>.
     *
     ...
     *
     * @return $this|ItemAportacion The current object, for fluid interface
     */
     ...
     * @return $this|\ItemAportacion The current object (for fluent API support)
     */
    public function setId($v)
    {
        ...
        if ($this->id !== $v) {
            $this->id = $v;
            $this->modifiedColumns[ItemAportacionTableMap::COL_ID] = true;
        }
    ...
    public function setMonto($v)
    {
        ...
        if ($this->monto !== $v) {
            $this->monto = $v;
            $this->modifiedColumns[ItemAportacionTableMap::COL_MONTO] = true;
        }
    ...
    public function setFechaInicio($v)
    {
        ...
        if ($this->fecha_inicio !== null || $dt !== null) {
            if ($dt !== $this->fecha_inicio) {
                ...
                $this->modifiedColumns[ItemAportacionTableMap::COL_FECHA_INICIO] = true;
            }
    ...
    public function setFechaFin($v)
    {
        ...
        if ($this->fecha_fin !== null || $dt !== null) {
            if ($dt !== $this->fecha_fin) {
                $this->fecha_fin = $dt;
                $this->modifiedColumns[ItemAportacionTableMap::COL_FECHA_FIN] = true;
            }
        } // if either are not null
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 0 + $startcol : ItemAportacionTableMap::translateFieldName('Id', TableMap::TYPE_PHPNAME, $indexType)];
            $this->id = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : ItemAportacionTableMap::translateFieldName('Monto', TableMap::TYPE_PHPNAME, $indexType)];
            $this->monto = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : ItemAportacionTableMap::translateFieldName('FechaInicio', TableMap::TYPE_PHPNAME, $indexType)];
            if ($col === '0000-00-00') {
                $col = null;
            }
            $this->fecha_inicio = (null !== $col) ? PropelDateTime::newInstance($col, null, 'DateTime') : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 3 + $startcol : ItemAportacionTableMap::translateFieldName('FechaFin', TableMap::TYPE_PHPNAME, $indexType)];
            if ($col === '0000-00-00') {
                $col = null;
            }
            ...
            return $startcol + 4; // 4 = ItemAportacionTableMap::NUM_HYDRATE_COLUMNS.

        } catch (Exception $e) {
            throw new PropelException(sprintf('Error populating %s object', '\\ItemAportacion'), 0, $e);
        }
    }
    ...
    public function reload($deep = false, ConnectionInterface $con = null)
    {
        ...
        $dataFetcher = ChildItemAportacionQuery::create(null, $this->buildPkeyCriteria())->setFormatter(ModelCriteria::FORMAT_STATEMENT)->find($con);
     ...
     * @return void
     * @throws PropelException
     * @see ItemAportacion::setDeleted()
     * @see ItemAportacion::isDeleted()
     */
    public function delete(ConnectionInterface $con = null)
    {
        ...
        $con->transaction(function () use ($con) {
            $deleteQuery = ChildItemAportacionQuery::create()
    ...
    public function save(ConnectionInterface $con = null)
    {
        ...
        return $con->transaction(function () use ($con) {
            ...
            if ($ret) {
                ...
                $this->postSave($con);
                ItemAportacionTableMap::addInstanceToPool($this);
            } else {
                $affectedRows = 0;
            }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(ItemAportacionTableMap::COL_ID)) {
            $modifiedColumns[':p' . $index++]  = 'id';
        }
        if ($this->isColumnModified(ItemAportacionTableMap::COL_MONTO)) {
            $modifiedColumns[':p' . $index++]  = 'monto';
        }
        if ($this->isColumnModified(ItemAportacionTableMap::COL_FECHA_INICIO)) {
            $modifiedColumns[':p' . $index++]  = 'fecha_inicio';
        }
        if ($this->isColumnModified(ItemAportacionTableMap::COL_FECHA_FIN)) {
            $modifiedColumns[':p' . $index++]  = 'fecha_fin';
        }

        $sql = sprintf(
            'INSERT INTO item_aportacion (%s) VALUES (%s)',
    ...
    {
        $pos = ItemAportacionTableMap::translateFieldName($name, $type, TableMap::TYPE_NUM);
    ...
    public function toArray($keyType = TableMap::TYPE_PHPNAME, $includeLazyLoadColumns = true, $alreadyDumpedObjects = array(), $includeForeignObjects = false)
    {
        ...
        if (isset($alreadyDumpedObjects['ItemAportacion'][$this->hashCode()])) {
            return '*RECURSION*';
        }
        $alreadyDumpedObjects['ItemAportacion'][$this->hashCode()] = true;
        $keys = ItemAportacionTableMap::getFieldNames($keyType);
        $result = array(
            $keys[0] => $this->getId(),
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(ItemAportacionTableMap::COL_ID)) {
            $criteria->add(ItemAportacionTableMap::COL_ID, $this->id);
        }
        if ($this->isColumnModified(ItemAportacionTableMap::COL_MONTO)) {
            $criteria->add(ItemAportacionTableMap::COL_MONTO, $this->monto);
        }
        if ($this->isColumnModified(ItemAportacionTableMap::COL_FECHA_INICIO)) {
            $criteria->add(ItemAportacionTableMap::COL_FECHA_INICIO, $this->fecha_inicio);
        }
        if ($this->isColumnModified(ItemAportacionTableMap::COL_FECHA_FIN)) {
            $criteria->add(ItemAportacionTableMap::COL_FECHA_FIN, $this->fecha_fin);
        }
    ...
    {
        $criteria = ChildItemAportacionQuery::create();
        $criteria->add(ItemAportacionTableMap::COL_ID, $this->id);
     ...
     * objects.
     *
     * @param      object $copyObj An object of \ItemAportacion (or compatible) type.
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \ItemAportacion Clone of current object.
     * @throws PropelException
     */
     ...
     * If this ChildItemAportacion is new, it will return
     ...
     * @param      ConnectionInterface $con Optional connection object
     * @return $this|ChildItemAportacion The current object (for fluent API support)
     */
    public function setItems(Collection $items, ConnectionInterface $con = null)
    {
        ...
        foreach ($itemsToDelete as $itemRemoved) {
            $itemRemoved->setItemAportacion(null);
        }
     ...
     * Otherwise if this ItemAportacion is new, it will return
     * an empty collection; or if this ItemAportacion has previously
     * been saved, it will retrieve related Items from storage.
     *
     * This method is protected by default in order to keep the public
     * api reasonable.  You can provide public methods for those you
     * actually need in ItemAportacion.
     *
     * @param      Criteria $criteria optional Criteria object to narrow the query
     * @param      ConnectionInterface $con optional connection object
     ...
     * @return ObjectCollection|ChildItem[] List of ChildItem objects
     */
    public function getItemsJoinProyecto(Criteria $criteria = null, ConnectionInterface $con = null, $joinBehavior = Criteria::LEFT_JOIN)
    {
        $query = ChildItemQuery::create(null, $criteria);
    ...
    {
        return (string) $this->exportTo(ItemAportacionTableMap::DEFAULT_STRING_FORMAT);
    }
